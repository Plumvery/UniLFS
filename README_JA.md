...
| Pull | ローカルに無い/古くなったファイルをダウンロード |
| Restore Modified | ローカル変更・衝突をマニフェストの版で上書き（確認ダイアログあり） |
| Track / Untrack Selected | `Assets > UniLFS` の右クリックメニューと同じ |

状態表示: **up to date**（マニフェストと一致し、ストレージ上の blob も確認済み）/ **not pushed**（マニフェストとは一致するが、このマシンからのアップロードが確認できていない。Track しただけで未 Push の場合など）/ **modified**（自分のローカル変更が未Push）/ **outdated**（他の人がより新しい版をPush済み。Pullが必要）/ **conflicted**（前回同期以降、こちらとマニフェストの両方が変わった）/ **missing**（Pullが必要）。

**modified** と **outdated** の区別には3つ目の材料が要ります。どちらも「ローカルがマニフェストと違う」だけの状態なのに、押すべきボタンは正反対だからです。UniLFS は各ファイルが「このマシンで最後にマニフェストと一致していたハッシュ」を `Library/UniLFS/` に記録し、それと比較します（マージベースがdiffのどちら側が動いたかを決めるのと同じ考え方です）。**conflicted** はこの比較で「両側が動いた」と出た状態で、手動解決が必要です: **Restore Modified**（マニフェスト側を採用）か、**Track Selected** → Push（ローカル側を採用）。
...
**Auto Pull** — エディタの起動時・フォーカス復帰時（＝`git pull`した直後）に、マニフェストファイル自体が前回チェックから変わっている場合だけ追跡ファイルを再チェック。欠けている、または古くなっているファイルがあれば、設定に応じて **Ask**（デフォルト・ダイアログ）/ **Automatic**（バックグラウンドでダウンロード）/ **Off**（Console警告のみ）。
